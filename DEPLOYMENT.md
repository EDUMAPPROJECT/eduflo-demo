...
# supabase functions deploy update-auth-settings
```
...
4. **Secrets** 탭에서 아래 변수 추가 (배포하는 함수에 따라 필요 시 설정)

**send-verification-email / update-auth-settings 등 공통:**
...
1. **대시보드에서 확인**  
   Supabase Dashboard → **Edge Functions** 메뉴에서 배포한 함수들이 목록에 있고 상태가 정상인지 봅니다.
